...
export function initializeTokenRefresh(): void {
  ...
  if (expiresAt && refreshTokenStr) {
    ...
    // If token has more than 30s remaining, schedule it.
    // Otherwise (expired or very close), refresh immediately.
    // This handles local time drift or long inactive periods.
